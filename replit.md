...
- **40 Under 40 Local Storage**: `data/40under40-applications.jsonl` and `data/40under40-uploads/` are per-container and wiped on WPE Atlas redeploys, so the signed document links in the webhook payload eventually 404. That is why the webhook also carries the file bytes themselves — the external system, not this container, is the durable archive.
