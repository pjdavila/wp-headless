...
- **Optional Env Vars**:
    ...
    - `R2_PUBLIC_BUCKET` + `R2_PUBLIC_BASE_URL` (optional *separate* public bucket and its URL, e.g. `https://pub-xxxx.r2.dev`; used only for the candidate photo. Both are required and the bucket must differ from `R2_BUCKET`, otherwise the photo is served through an expiring presigned link like the résumé)
