...
import { rewriteWpImageSrc } from "./rewriteWpContentImageUrls.js";
...
const NEWSLETTER_MAX_POSTS = 6;

/**
 * Alerts the team inbox when a scheduled newsletter send fails. Non-fatal by
 * design: alerting must never mask or replace the original failure. Returns
 * true when the alert was handed to Resend, false otherwise.
 */
export async function sendNewsletterFailureAlert({ step, error, trigger }) {
  const apiKey = process.env.RESEND_API_KEY;
  const to =
    process.env.NEWSLETTER_NOTIFY_EMAIL || process.env.PRINT_EDITION_NOTIFY_EMAIL;
  if (!apiKey || !to) {
    console.error(
      "[daily-newsletter] Failure alert skipped — RESEND_API_KEY or notify email missing",
    );
    return false;
  }

  const when = formatEnglishDate(new Date());
  const html = `
<!DOCTYPE html>
<html lang="en">
...
<body style="margin:0;padding:24px;background-color:#f4f4f5;font-family:Helvetica,Arial,sans-serif;">
  <div style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:8px;padding:24px;">
    <h2 style="margin:0 0 12px;font-size:18px;color:#b91c1c;">Daily newsletter failed to send</h2>
    <p style="margin:0 0 8px;font-size:14px;color:#3f3f46;">
      The 5pm daily newsletter could not be sent on <strong>${when}</strong>.
    </p>
    <p style="margin:0 0 8px;font-size:14px;color:#3f3f46;">
      <strong>Trigger:</strong> ${trigger || "unknown"}<br />
      <strong>Failed step:</strong> ${step || "unknown"}<br />
      <strong>Error:</strong> <code>${String(error || "unknown")}</code>
    </p>
    <p style="margin:16px 0 0;font-size:13px;color:#71717a;">
      Subscribers did not receive today's edition. Check the server logs for
      "[daily-newsletter]" around the failure time for the upstream detail
      (WordPress or Moosend).
    </p>
  </div>
</body>
</html>`.trim();
  ...
  try {
    const resp = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [to],
        subject: `[CB] Daily newsletter failed: ${error || step || "unknown error"}`,
        html,
      }),
    });
    if (!resp.ok) {
      console.error(
        "[daily-newsletter] Failure alert email rejected:",
        resp.status,
      );
      return false;
    }
    console.info(`[daily-newsletter] Failure alert emailed to ${to}`);
    return true;
  } catch (err) {
    console.error("[daily-newsletter] Failure alert send failed:", err.message);
    return false;
  }
}

const LEAD_POSTS_QUERY = `
  query DailyNewsletterPosts {
    posts(
      first: 50
      where: { tagSlugIn: ["portada", "lead"], orderby: { field: DATE, order: DESC } }
    ) {
      nodes {
        id
        title
...
export async function fetchLeadPostsLast24h() {
  ...
  const recent = nodes.filter((post) => {
    if (!post.dateGmt) return false;
    const ts = Date.parse(`${post.dateGmt}Z`);
    return (
      Number.isFinite(ts) && ts >= cutoff && ts <= Date.now() + 5 * 60 * 1000
    );
  });

  const isPortada = (post) =>
    (post.tags?.nodes || []).some((t) => t.slug === "portada");

  // Stable partition keeps each group in DATE DESC order (query order).
  const portada = recent.filter(isPortada);
  const lead = recent.filter((post) => !isPortada(post));

  return [...portada, ...lead].slice(0, NEWSLETTER_MAX_POSTS);
}
...
export async function sendDailyNewsletterCampaign({
  subject,
  html,
  date = new Date(),
}) {
