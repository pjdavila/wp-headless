...
import { isPendingSourceError } from "./providers/pending.js";
import {
  buildSnapshot,
...
export async function runIngestion({
  ...
  for (const indicator of targets) {
    ...
    try {
      ...
      // Sources awaiting a manual upload report "pending", not "error":
      // the dashboard keeps showing real metadata with no values, and an
      // existing last-good snapshot is never clobbered.
      if (isPendingSourceError(err)) {
        const previous = await readSnapshot(indicator.id).catch(() => null);
