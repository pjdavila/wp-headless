...
// Variants with their own hand-written copy. `lang` only drives the <html lang>
// attribute; every string below is already written in that language.
const VARIANT_COPY = {
  "print-edition-interest": {
    ...
    footerNote:
      "© {year} Caribbean Business — Una publicación de Vision News Media",
  },
  "print-subscription": {
    lang: "en",
    subject: "Your print edition subscription is active",
    heading: "Welcome to the print edition",
    greetingFallback: "Hi there,",
    body: [
      "Your monthly subscription to the Caribbean Business print edition is active. Thank you for supporting business journalism made in Puerto Rico.",
      "You'll receive the next issue directly at the address you gave us. The $4.99 USD charge renews every month and you can cancel anytime from the subscription portal on our subscription page.",
      "If you need to update your shipping address or payment details, visit caribbean.business/subscription/ and use the “Manage subscription” section with this same email.",
    ],
    ctaLabel: "Visit Caribbean Business",
    footerNote: "© {year} Caribbean Business — A Vision News Media Publication",
  },
  "forty-under-40": {
    lang: "en",
    subject: "We received your 40 Under 40 · 2026 nomination",
    heading: "Your 40 Under 40 nomination is in",
    greetingFallback: "Hi there,",
    body: [
      "Thank you for entering Caribbean Business 40 Under 40 · 2026. We have received your submission and it is now with our editorial team.",
      "Every entry is reviewed by our editors. If we need anything else — an updated résumé, a higher-resolution photo, or a few extra details — we will reach out to you at this address.",
      "Honorees will be notified by email before the class of 2026 is announced. In the meantime, feel free to reply to this message with any questions.",
    ],
    ctaLabel: "Visit Caribbean Business",
    footerNote: "© {year} Caribbean Business — A Vision News Media Publication",
  },
...
function buildVariantHtml({ greeting, copy }) {
...
export async function sendWelcomeEmail({ email, name, variant } = {}) {
  ...
  if (variantCopy) {
    const salutation = variantCopy.lang === "en" ? "Hi" : "Hola";
    const greeting = firstName
      ? `${salutation} ${firstName},`
      : variantCopy.greetingFallback;
    html = buildVariantHtml({ greeting, copy: variantCopy });
