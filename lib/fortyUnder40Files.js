...
// field -> accepted content types -> canonical extension. Up to three
// recommendation letters can be attached per application.
export const UPLOAD_FIELDS = {
  recommendation1: RECOMMENDATION_SPEC,
  recommendation2: RECOMMENDATION_SPEC,
  recommendation3: RECOMMENDATION_SPEC,
};

const STORED_NAME_RE = /^[a-f0-9-]{36}-(recommendation[123])\.(pdf)$/;
...
export function validateUpload(field, value) {
  ...
  if (!extension) {
    return { ok: false, error: "The recommendation letter must be a PDF file." };
  }
